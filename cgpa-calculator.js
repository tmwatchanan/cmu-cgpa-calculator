...
function addNewCourseRow() {
  ...
  $("#myCourseLetterGrade-" + idxMyCourse).on(
    "change keydown paste input",
    function() {
      // console.log("-> " + $( this ).val() );
      CalculateExpectedGrades();
...
function CalculateExpectedGrades() {
  ...
  $("#nextGPA").text(isNaN(nextGPA) ? (0.0).toFixed(2) : nextGPA.toFixed(2));
  ...
  $("#expectedEnrolledCredits").text(
    regCmuEnrolledCredits +
      nextCredits +
      nextCreditsEnrolledGradeSorU +
      nextCreditsGradeP
  );
  $("#expectedGotCredits").text(
    regCmuGotCredits + nextCredits + nextCreditsGotGradeSorU - nextCreditsGradeF
  );
...
function FindNextEnrolledCourseList(studentId, lastSemester, lastYear) {
  ...
  if (lastSemester == "1") {
    nextSemester = "2";
    nextYear = lastYear;
    courseListObj = GetEnrolledCourseListOfStudentIdFromAPI(
      studentId,
      nextSemester,
      nextYear
    );
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (responseObj.status == false && semester != "3") {
        semester = "3";
        responseObj = GetEnrolledCourseListOfStudentIdFromAPI(
          studentId,
          semester,
          year
        );
  ...
  if (courseNo.length == 6) {
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        var courseCreditField = document.getElementById(
          "myCourseCredit-" + noIdx
        );
...
function FillTableWithEnrolledCourses(courseObj) {
  ...
  $("#table-main")
    .find("tr:not(:first)")
    .remove(); // clear all TR expect header row
...
if (document.title.indexOf(pageTitle) != -1) {
  ...
  var tableHeaderText = $("html body div div div")
    .find("h3")
    .text();
  ...
  if (yearNum == firstYear + 4 && tableHeaderText.search(" 3 ") != -1) {
  ...
  for (var i = 1; i < numTableFound; i += 2) {
    var numFound = $("html body div div div > table")
      .eq(i)
      .find("tbody")
      .find("tr").length;
    ...
    for (var j = 0; j < numFound; j++) {
      var credit = $("html body div div div > table")
        .eq(i)
        .find("tbody")
        .find("tr")
        .eq(j)
        ...
        .get();
      credits.push(...credit);
      //   console.log("Credit[" + (j + 1) + "] = " + credit);

      var letterGrade = $("html body div div div > table")
        .eq(i)
        .find("tbody")
        .find("tr")
        .eq(j)
  ...
  regCmuEnrolledCredits = $("html body div > table")
    .eq(lastTable)
    .find("tbody tr")
    .eq(1)
    .find("td")
  ...
  regCmuGotCredits = $("html body div > table")
    .eq(lastTable)
    .find("tbody tr")
    .eq(1)
    .find("td")
